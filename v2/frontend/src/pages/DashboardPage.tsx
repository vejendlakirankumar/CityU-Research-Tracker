...
import { FileText, ShieldCheck, Clock, CheckCircle, AlertTriangle } from 'lucide-react'
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {stats.map((stat) => {
            ...
            return (
              <div
                key={stat.key}
                className={`bg-white rounded-xl border shadow-sm p-5 ${isAlert ? 'border-red-200' : 'border-gray-200'}`}
              >
                <div className="flex items-center justify-between">
                  <p className="text-sm font-medium text-gray-500">{stat.label}</p>
                  <Icon className={`w-5 h-5 ${color}`} />
                </div>
                <p className={`mt-3 text-3xl font-bold ${isAlert ? 'text-red-600' : 'text-gray-900'}`}>
                  {stat.value}
                </p>
              </div>
            )
