	...
	public static function derive_submission_status( $submission ) {
		...
		if ( $_is_gated ) {
			...
			if ( ! empty( $_releases ) && ! $_release_superseded ) {
				$_check_rel = end( $_releases );
				if ( (string) ( $_check_rel['decision'] ?? '' ) === 'Approved' ) {
					foreach ( $_higher_stages as $_hs_chk ) {
						if ( $_hs_chk['skipped'] ?? false ) continue;
						if ( empty( $_hs_chk['reviewers'] ) ) continue;
						$_hs_chk_decs = is_array( $_hs_chk['decisions'] ?? null ) ? $_hs_chk['decisions'] : array();
						foreach ( $_hs_chk['reviewers'] as $_hcr ) {
							$_hck  = strtolower( trim( (string) ( $_hcr['email'] ?? '' ) ) );
							$_hcd  = strtolower( (string) ( $_hs_chk_decs[ $_hck ] ?? '' ) );
							if ( empty( $_hcd ) || $_hcd === 'pending' ) { $_release_superseded = true; break 2; }
						}
					}
				}
			}
			...
			if ( ! empty( $_releases ) && ! $_release_superseded ) {
				...
				// No release yet (or release was invalid) — check higher-stage progress.
				// Scan in order: find the first stage that is still pending.
				// Once all assigned stages are done, show 'Reviewing <last stage> Feedback'.
				$_hs_pending_name = '';
				$_hs_last_done    = '';
				foreach ( $_higher_stages as $_hs ) {
					if ( $_hs['skipped'] ?? false ) continue;
					$_hs_reviewers = $_hs['reviewers'] ?? array();
					if ( empty( $_hs_reviewers ) ) continue;
					...
					foreach ( $_hs_reviewers as $_hr ) {
						...
						$_hdec = strtolower( (string) ( $_hs_decisions[ $_hek ] ?? '' ) );
						if ( empty( $_hdec ) || $_hdec === 'pending' ) { $_hs_all_decided = false; break; }
					}
					if ( $_hs_all_decided ) {
						$_hs_last_done = trim( (string) ( $_hs['stageName'] ?? '' ) );
					} else {
						$_hs_pending_name = trim( (string) ( $_hs['stageName'] ?? '' ) );
						break;
					}
				}
				if ( $_hs_pending_name ) {
				...
				if ( $_hs_last_done ) {
					$_suffix = ': Reviewing ' . $_hs_last_done . ' Feedback';
