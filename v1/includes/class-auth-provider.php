...
class RRP_Auth_Provider {
	...
	public static function handle_entra_callback( WP_REST_Request $request ) {
		...
		// 6b. Validate critical token claims to prevent token replay and cross-tenant/app spoofing.
		// exp — reject any token whose expiry timestamp is in the past.
		if ( isset( $claims['_exp'] ) && $claims['_exp'] < time() ) {
			wp_die(
				esc_html__( 'SSO token has expired. Please try logging in again.', 'rrp' ),
				esc_html__( 'Authentication Error', 'rrp' ),
				array( 'response' => 401 )
			);
		}
		// iss — must be the expected Entra v2.0 issuer for this specific tenant.
		$_expected_iss = 'https://login.microsoftonline.com/' . $tenant_id . '/v2.0';
		if ( isset( $claims['_iss'] ) && $claims['_iss'] !== $_expected_iss ) {
			wp_die(
				esc_html__( 'SSO token issuer mismatch. Authentication rejected.', 'rrp' ),
				esc_html__( 'Authentication Error', 'rrp' ),
				array( 'response' => 403 )
			);
		}
		// aud — must be this application's client_id to prevent confused-deputy attacks.
		if ( isset( $claims['_aud'] ) && $claims['_aud'] !== $client_id ) {
			wp_die(
				esc_html__( 'SSO token audience mismatch. Authentication rejected.', 'rrp' ),
				esc_html__( 'Authentication Error', 'rrp' ),
				array( 'response' => 403 )
			);
		}
		// Strip internal validation keys before passing claims to map_entra_user().
		unset( $claims['_exp'], $claims['_iss'], $claims['_aud'] );
	...
	private static function decode_jwt_payload( string $jwt ): ?array {
		...
		return array(
			...
			// Validation claims — kept as raw values (not sanitized) so they can
			// be compared precisely against expected values in handle_entra_callback().
			// These keys are prefixed with '_' and stripped before passing to map_entra_user().
			'_exp'       => isset( $claims['exp'] ) ? (int) $claims['exp']    : null,
			'_iss'       => isset( $claims['iss'] ) ? (string) $claims['iss'] : null,
			'_aud'       => isset( $claims['aud'] ) ? (string) $claims['aud'] : null,
		);
	}
