(function () {
  ...
  var _v1ReadOnly = !!(window.RRP && window.RRP.v1ReadOnly);
  var _v2Url      = (window.RRP && window.RRP.v2Url) || '';

  if (_v1ReadOnly) {
    document.addEventListener('DOMContentLoaded', function () {
      var banner = document.createElement('div');
      banner.id = 'rrp-readonly-banner';
      banner.style.cssText = [
        'position:fixed', 'top:0', 'left:0', 'right:0', 'z-index:99999',
        'background:#b45309', 'color:#fff', 'padding:.65rem 1.25rem',
        'display:flex', 'align-items:center', 'justify-content:center',
        'gap:.75rem', 'font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif',
        'font-size:.9rem', 'font-weight:500', 'box-shadow:0 2px 8px rgba(0,0,0,.3)'
      ].join(';');
      var msg = '\u26A0\uFE0F This portal is now <strong>read-only</strong>. New submissions and edits must be made in the new portal.';
      if (_v2Url) {
        msg += ' <a href="' + _v2Url + '" style="color:#fef3c7;text-decoration:underline;font-weight:700;">Open new portal &rarr;</a>';
      }
      banner.innerHTML = msg;
      document.body.insertBefore(banner, document.body.firstChild);
      // Push content below fixed banner
      var wrap = document.getElementById('research-review-portal');
      if (wrap) { wrap.style.paddingTop = '48px'; }
    });
  }
