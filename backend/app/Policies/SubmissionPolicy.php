...
class SubmissionPolicy
{
    ...
    /** Owner can withdraw own submission at any non-terminal status; admin/coordinator can also withdraw. */
    public function withdraw(User $user, Submission $submission): bool
    {
        ...
        if ($this->isAdminOrCoordinator($user)) {
            return !in_array($submission->status, $terminal);
        }

        return $submission->submitter_id === $user->id
            && !in_array($submission->status, $terminal);
    }

    /** Admin or coordinator can cancel any non-terminal submission. */
    public function cancel(User $user, Submission $submission): bool
    {
        if (!$this->isAdminOrCoordinator($user)) {
            return false;
        }
